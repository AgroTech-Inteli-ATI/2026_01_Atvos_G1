...
> As alternativas locais estão documentadas na seção final.

---

## Fontes Disponíveis

### Fonte 1 — Correção de Talhões para Unificação
...
| `CHAVE` | str | Chave legível no formato NUM-SETOR-TALHAO | — | 0% |
...
| `UndGerencial` | str | Unidade gerencial, equivalente a `DESC_EMPRESA` | — | 0% |
...
| `DESC_AMBIENTE` | str | Descrição do ambiente de produção | — | ~36% (geo) |
| `DE_TP_SOLO` | str | Descrição do tipo de solo | — | 0% |
| `ESPAC` | str | Espaçamento de plantio | — | 0% |
| `AREA_PROD` | float | Área de produção (ha) — **imputada** | — | 0% após a Silver |
| `TCH_PROD` | float | Toneladas de cana por hectare estimadas — **imputada** | — | 0% após a Silver |
| `TON_ESTIM` | float | Toneladas estimadas de produção — **imputada** | — | 0% após a Silver |
...
| `CANA_ENT` | float | Cana entregue na usina (t) | — | ~99% (negócio) |
...
| `LATITUDE` | str | Latitude do centroide do talhão | — | ~18,5% (geo) |
| `LONGITUDE` | str | Longitude do centroide do talhão | — | ~18,5% (geo) |
...
Inventario_part_1
Inventario_part_2
Inventario_part_3  -> Unir por CHAVESIG -> Inventario Completo
Inventario_part_4
        |
